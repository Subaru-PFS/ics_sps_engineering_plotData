...
class Graph(FigureCanvas):
    ...
    def addordelCurve(self, checkbox, label, type, ylabel, unit, row, column, combo, spinbox=None):
        if not self.rdy_check:
            if checkbox.isChecked():
                if self.addCurve(label, type, ylabel, unit, row, column, combo, spinbox):
                    return 1
                else:
                    self.rdy_check = True
    ...
    def addCurve(self, label, type, ylabel, unit, row, column, combo, spinbox):
        ax = self.getAxe(type)
        if ax is not None:
            if not spinbox:
                new_curve = Curve(self.parent, self, label, type, ylabel, unit, row, column, combo)
            else:
                new_curve = Derivate(self.parent, self, label, type, ylabel, unit, row, column, combo, spinbox)
