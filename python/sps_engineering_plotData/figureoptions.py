...
def figure_edit(axes, parent=None):
    ...
    figure.lock()
    if figure.graph.toolbar.getViewEmpty():
        figure.graph.toolbar.push_current()
    ...
    def apply_callback(data):
        ...
        for index, curve in enumerate(curves):
            ...
            line2curve = axes.get_figure().graph.plotWindow.line2Curve
            try:
                line2curve[line].updateProp(yscale=yscale,
                                            label=label,
                                            ylabel=ylabel,
                                            color=color)
            except KeyError:
        ...
        axes.get_figure().graph.checkScales()
    ...
    if data is not None:
        apply_callback(data)
        axes.get_figure().graph.toolbar.push_current()
        figure.unlock()
